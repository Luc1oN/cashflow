...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ToastContext.Provider value={{ toast }}>
      {children}
      <div
        className="pointer-events-none fixed left-1/2 z-[60] flex w-full max-w-sm -translate-x-1/2 flex-col items-center gap-2 px-4 rail:!bottom-6"
        // Sits above the mobile tab bar plus the home indicator; on the desktop
        // layout (no tab bar) it drops back to a normal bottom margin.
