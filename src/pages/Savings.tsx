...
export default function Savings() {
  ...
  const selected = rows.find((g) => g.id === selectedId) ?? rows.find((g) => g.is_active) ?? rows[0] ?? null
  ...
  return (
    <div>
      <PageHeader
        title="Savings goals"
        subtitle="Each pot, projected forward at your per-payslip rate"
        action={<Button onClick={() => open('new')}>Add goal</Button>}
      />
          ...
          <div className="flex flex-wrap gap-2">
            {rows.map((g) => (
              <button
                ...
                className={`rounded-lg border px-3 py-1.5 text-sm font-medium ${selected?.id === g.id ? 'border-accent bg-accent-soft text-accent' : 'border-line bg-surface text-slate2 hover:text-ink'}`}
              >
                {g.name}{!g.is_active && ' (paused)'}
              </button>
            ))}
          </div>
                  ...
                  {projection.target != null ? (
                    <>
                      ...
                      <p className="mt-1 font-num text-xl font-semibold text-ink">
                        {projection.reachDate ? format(parseISO(projection.reachDate), 'd MMM yyyy') : 'Not on track'}
                      </p>
                      <p className="text-xs text-slate2">
                        {projection.reachDate ? `Target ${money(projection.target)}` : `Add a per-payslip amount to reach ${money(projection.target)}`}
                      </p>
                    </>
                  ) : (
                    <>
              ...
              <Card className="p-5">
                <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge tone={selected.deduction_type === 'pre_tax' ? 'warn' : 'neutral'}>
                      {selected.deduction_type === 'pre_tax' ? 'Pre-tax' : 'Post-tax'}
                    </Badge>
                ...
                <div className="mb-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate2">
                  ...
                  {projection.target != null && (
                    <span className="flex items-center gap-1.5"><span className="inline-block h-0 w-4 border-t-2 border-dashed align-middle" style={{ borderColor: colors.mossdeep }} /> Target</span>
                  )}
                </div>
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={projection.points} margin={{ top: 8, right: 8, bottom: 0, left: 8 }}>
                      ...
                      <YAxis
                        domain={[0, Math.ceil(Math.max(projection.maxBalance, projection.target ?? 0) * 1.05)]}
                        tickFormatter={(v: number) => moneyShort(v)}
                        tick={{ fontSize: 11, fill: colors.slate, fontFamily: 'Geist Mono' }}
                        tickLine={false} axisLine={false} width={72}
                      />
                      <Tooltip
                        formatter={(v) => [money(Number(v)), 'Projected balance']}
                      ...
                      />
                      {projection.target != null && (
                        <ReferenceLine y={projection.target} stroke={colors.mossdeep} strokeDasharray="5 4" label={{ value: `Target ${moneyShort(projection.target)}`, position: 'insideTopRight', fontSize: 10, fill: colors.mossdeep }} />
                      )}
